...
def detect(save_img=True, save_num=False, save_txt=True, save_conf=False, plot_label=True, plot_conf=True,
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if len(det):
            ...
            if save_txt:
                txt_path = os.path.join(save_dir, os.path.basename(path).split('.')[0]) + \
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                if classify:
                    if int(cls) == 0:
                        label = classifier.inference(im0s_copy, (cls, *xywh))
                ...
                #     if save_num:
                #         plot_car_num(im0s, len(det), color=[0, 0, 255])
            if save_txt:
                f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                        default='/project/objectdetection_shawn/weights/yolo_weights/smoking_detection_v5s_conv+aug3/weights/best.pt')
    parser.add_argument('--source', type=str,
                        # default='/data/dataset/yolo_dataset/Fall/original/FallDataset_A')
                        # default='/mnt/projects/ObjectDetection/dataset/fall/fall_dataset/')
                        default='/project/data/smoking_data_wenny/cigarette_kaggle/cigarette-dataset/train')
    parser.add_argument('--save_dir', type=str,
                        # default='/data/dataset/yolo_dataset/Fall/original/FallDataset_A')
                        # default='/mnt/projects/ObjectDetection/dataset/fall/fall_cls')
                        default='/project/data/smoking_data_wenny/cigarette_kaggle/cigarette-dataset/train_tmp')
                        # default='/mnt/projects/ObjectDetection/dataset/motor/sample_results')
