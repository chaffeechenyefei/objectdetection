...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        self.grid = [torch.zeros(1)] * self.nl  # init grid
        a = torch.tensor(anchors).float().view(self.nl, -1, 2)
        b = torch.tensor(anchors).float()
        self.register_buffer('anchors', a)  # shape(nl,na,2)
        self.register_buffer('anchor_grid', b.reshape(self.nl, 1, -1, 1, 1, 2))  # shape(nl,1,na,1,1,2)
        # self.anchor_grid = nn.Parameter(b.reshape(self.nl, 1, -1, 1, 1, 2), requires_grad=False)
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch)  # output conv
        self.singleOutput = False
        self.grid0 = None
        self.grid1 = None
    ...
    def forward(self, x):
        ...
        self.training |= self.export
        print(self.training, self.export, 'None' if not hasattr(self, 'singleOutput') else self.singleOutput)
        # print(self.nl)
        # print(self.anchor_grid)
        # if hasattr(self, 'singleOutput') and self.singleOutput:
        #     output = []
        #     for i in range(self.nl):
        #         x[i] = self.m[i](x[i])  # conv
        #         bs, _, ny, nx = x[i].shape  # x(bs,255,20,20) to x(bs,3,20,20,85)
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
                # print('grid', self.grid[i].shape)
                # print('stride', self.stride[i])
                # print('anchor_grid', self.anchor_grid[i].shape, self.anchor_grid[i])
                """
                    xy = (y[..., 0:2] * 2. - 0.5 + self.grid[i]) * self.stride[i]
                    wh = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i].reshape(1, -1, 1, 1, 2)
                    conf = y[..., 4:]
                    # print(xy.shape,wh.shape,conf.shape)
                    xy = xy.reshape(bs,-1,2)
                    wh = wh.reshape(bs,-1,2)
                    conf = conf.reshape(bs,-1,self.nc+1)
                    # print('xy,wh shape =', xy.shape, wh.shape)
                    y = torch.cat([xy, wh, conf], dim=-1)
                    z.append(y)
                    tried but failed!!!
                """
                # y = torch.cat([xy, wh], dim=4)

                # print(bs, self.no, y.shape)
                z.append(y.reshape(bs, -1, self.no))

        # for _z in z:
        #     print(_z.shape, self.no)

        # print(self.anchor_grid.reshape(self.nl,self.na,2))

        if self.training:
            if self.export:
                y = []
                for _x in x:
                    y.append(_x.reshape(1, -1))  # [1, na-h-w-d]
                y.append(self.anchor_grid.reshape(1, -1))  # [1, nl-na-2]
                return y
            else:
                return x
        else:
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None, singleOutput=False):  # model, input channels, number of classes
        ...
        print('Detect initialing')
        m = self.model[-1]  # Detect()
        if isinstance(m, Detect):
            m.set_single_output(singleOutput)
            s = 256  # 2x min stride
            #####这里有一个forward#####
